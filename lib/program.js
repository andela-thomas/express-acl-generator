(function() {
  ...
  function deleteMethod(currentArray, array) {
    var newArray = [];
    for (var value of currentArray) {
  ...
  module.exports = {
    ...
    remove: {
      group: function(fs, args) {
        ...
        if (files.indexOf(filename) === -1) {
          return log.error('EONT', 'file does not exist');
        }

        config = JSON.parse(fs.readFileSync(path));

        index = getIndexGroup(config, args.group);

        if (index === -1) {
          return log.error('Not found', 'Group ' + args.group +
            ' was not found, add and try again');
        }
        ...
        fs.writeFile(path, config, function(err) {
          ...
          return log.info('SUCCESS', 'Group removed succesfully');
        });
      ...
      policy: function(fs, args) {
        ...
        if (index === -1) {
          return log.error('Resource not found', ' Resource' + resource +
            ' was not found, add and try again');
        }

        config[index].permissions.splice(permissionIndex, 1);
        ...
        fs.writeFile(path, config, function(err) {
          ...
          return log.info('SUCCESS', 'Policy removed succesfully');
        });
      },
      methods: function(fs, args) {
        nameOfTheGroup = args.group;
        ...
        methods = typeof currentMethods === 'string' ? [] :
          deleteMethod(currentMethod, methods);
        ...
        fs.writeFile(path, config, function(err) {
          ...
          return log.info('SUCCESS', 'Method(s) removed succesfully');
        });
      }
    }
  };
